    ...
    <body class="bg-slate-950 font-sans antialiased flex items-center justify-center min-h-screen p-4">
        <div class="w-full max-w-md bg-slate-900 border border-slate-700 rounded-2xl p-8 shadow-2xl space-y-6 transition-all duration-700 ease-out transform translate-y-0 opacity-100 animate-in fade-in slide-in-from-bottom-8 duration-500" style="max-width: 28rem;">
            <div class="text-center">
                ...
                <h1 class="mt-5 text-2xl font-semibold text-slate-100">Create your account</h1>
                <p class="mt-2 text-sm text-slate-400">Join your apartment community.</p>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="password" type="password" name="password" required autocomplete="new-password" class="w-full mt-1.5 px-4 py-3 bg-slate-950 border border-slate-700 text-slate-100 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm transition-all duration-300 focus:scale-[1.01] focus:ring-4 focus:ring-indigo-500/20 focus:border-indigo-500 shadow-inner">
                    @error('password')
                        <p class="mt-2 text-sm text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="password_confirmation" class="block text-xs font-semibold text-slate-400 tracking-wider">CONFIRM PASSWORD</label>
                ...
                <button type="submit" class="w-full px-4 py-3 bg-indigo-600 text-white font-semibold rounded-xl shadow-lg text-sm tracking-wide transition-all duration-200 hover:bg-indigo-500 active:scale-95 transform hover:-translate-y-0.5">Register</button>
            </form>
            ...
            <p class="text-center text-sm text-slate-400">Already registered? <a href="{{ route('login') }}" class="text-sky-300 hover:text-sky-200">Log in</a></p>
        </div>
    </body>
</html>
